...
def _store_results(results: dict):
    ...
    # Report to Lockwood for cross-project health tracking
    _report_to_lockwood(results, run_id)


def _report_to_lockwood(results: dict, run_id: int):
    """Push test results to Lockwood for dashboard health sparklines."""
    ...
    try:
        payload = json.dumps({
            ...
            "git_branch": results.get("git_branch", ""),
        ...
        print(f"Lockwood report failed (non-fatal): {e}")
...
async def trigger_run(project: Optional[str] = None, trigger: str = "manual"):
    """Trigger test run. trigger param: 'manual', 'commit', 'dispatch'."""
    if _running:
    ...
    def _manual_run():
        ...
        try:
            ...
            for repo_dir in repos_path.iterdir():
                ...
                # Pull latest before running (ensures we test the commit)
                _git_pull(path)
