...
async def initialize_project(request: ProjectInitRequest):
    ...
    result = await project_manager.create_project(request.project_name, request.project_root)
    return result
...
async def attach_project(request: ProjectAttachRequest):
    ...
    try:
        ...
        if not project_path.exists():
            raise HTTPException(
                status_code=404, detail=f"Project not found: {request.project_path}"
            )
...
async def parse_prd_markdown(request: PRDReadRequest):
    ...
    try:
        ...
        if not prd_file.exists():
            return {"success": False, "prd_data": None, "message": "PRD not found in project"}
...
def _parse_markdown_to_prd(markdown_content: str, project_name: str) -> Dict:
    ...
    prd_data = {
        ...
        "overview": {"executive_summary": "", "goals": "", "target_users": ""},
        "features": [],
        "user_stories": [],
    ...
    title_match = re.search(
        r"^#\s+(.+?)(?:\s*-\s*Product Requirements|\s*$)",
        markdown_content,
    ...
    goals_match = re.search(
        r"##\s+(?:Core\s+Requirements|Goals)\s*\n(.*?)(?=\n##|\Z)",
        markdown_content,
        re.DOTALL | re.IGNORECASE,
    )
    if goals_match:
    ...
    if feature_sections:
        for section in feature_sections:
            ...
            priority_match = re.search(
                r"\*\*Priority:\*\*\s*(Critical|High|Medium|Low)", section, re.IGNORECASE
            )
            if priority_match:
            ...
            status_match = re.search(
                r"\*\*Status:\*\*\s*(Implemented|Partial|Planned)", section, re.IGNORECASE
            )
            if status_match:
        ...
        if features_match:
            ...
            for match in re.finditer(feature_pattern, features_text, re.DOTALL):
                ...
                # Extract acceptance criteria
                criteria_match = re.search(
                    r"\*\*Acceptance Criteria:\*\*\s*(.*?)(?=---|##|\Z)", content, re.DOTALL
                )
...
async def discover_features(request: FeatureDiscoveryRequest):
    ...
    try:
        ...
        prd_data["metadata"] = {
            ...
            "avg_confidence": (
                sum(f.confidence for f in features) / len(features) if features else 0
            ),
        }
