...
async def save_prd(request: PRDRequest):
    ...
    try:
        result = await workspace_manager.save_prd(request.project_name, request.content)
        return result
...
async def save_tasks(request: TaskRequest):
    ...
    try:
        result = await workspace_manager.save_tasks(request.project_name, request.tasks)
        return result
    except Exception as e:
...
async def save_context(request: ContextRequest):
    ...
    try:
        result = await workspace_manager.save_context(
            request.project_name, request.context_type, request.content
        )
        return result
    except Exception as e:
...
"""
    ...
    async def run_build(project_name: str) -> dict:
        ...
        try:
            ...
            # Log output
            log_file = (
                f"/Users/byronhudson/Projects/BuildRunner3/workspace/logs/{project_name}_build.log"
            )
