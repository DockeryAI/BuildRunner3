...
async def tail_log_file(websocket: WebSocket, filename: str):
    ...
    if not filepath.exists():
        await websocket.send_json({"type": "error", "message": f"Log file {filename} not found"})
        return
...
async def execute_and_stream(command: str):
    ...
    try:
        ...
        await asyncio.gather(
            stream_output(process.stdout, False), stream_output(process.stderr, True)
        )
        ...
        await manager.send_status_update(
            "command_complete", {"command": command, "return_code": process.returncode}
        )

    except Exception as e:
        logger.error(f"Command execution error: {e}")
        await manager.send_command_output(command=command, output=None, error=str(e))
