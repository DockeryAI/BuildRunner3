...
## ⚠️ CRITICAL CORRECTION (Updated 2024-11-22)

### Correct PostgreSQL RLS Syntax

**IMPORTANT**: Previous versions of this guide contained incorrect information about INSERT policies. Here is the **CORRECT** syntax:

| Operation | Required Clauses | Explanation |
|-----------|-----------------|-------------|
| **SELECT** | `USING` only | Filters which existing rows can be read |
| **INSERT** | `WITH CHECK` only | Validates new rows being inserted |
| **UPDATE** | Both `USING` and `WITH CHECK` | USING: filters which rows can be updated<br>WITH CHECK: validates the updated values |
| **DELETE** | `USING` only | Filters which rows can be deleted |

### Correct Implementation Examples

```sql
-- ✅ CORRECT: INSERT with WITH CHECK only
CREATE POLICY "insert_items" ON items
FOR INSERT
WITH CHECK (user_id = auth.uid());  -- Only WITH CHECK for INSERT

-- ❌ WRONG: INSERT with USING (this will cause errors)
CREATE POLICY "insert_items" ON items
FOR INSERT
USING (user_id = auth.uid());  -- WRONG! INSERT doesn't use USING
```

### Supabase INSERT with .select() Pattern

When using Supabase's `.insert().select()` pattern, you need **TWO separate policies**:

```javascript
// This operation needs both INSERT and SELECT policies
const { data, error } = await supabase
  .from('items')
...
Required policies:
```sql
-- 1. INSERT policy to allow creating
CREATE POLICY "create_items" ON items
FOR INSERT
WITH CHECK (auth.uid() IS NOT NULL);

-- 2. SELECT policy to allow reading the returned data
CREATE POLICY "read_own_items" ON items
FOR SELECT
USING (user_id = auth.uid());
```

### Recent Fixes Applied to Synapse Project

1. **location_detection_cache** - Corrected to use proper RLS syntax
2. **All dashboard tables** - Updated with correct policy structure
3. **PostgREST cache** - Forced reload after policy corrections

### Development vs Production Policies

**Current Development Setup (TEMPORARY):**
- Using permissive `TO public` policies
- `USING (true)` and `WITH CHECK (true)` for testing

**Production Requirements:**
- Replace with proper authentication checks
- Implement tenant isolation
- Use JWT claims for user identification
- Never use `(true)` conditions in production

---

*This correction supersedes any conflicting information in this guide.*
